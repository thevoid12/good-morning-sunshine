...
const (
	...
	OWNER_SCHEMA = "CREATE TABLE IF NOT EXISTS owner (id TEXT NOT NULL,email_id TEXT NOT NULL,rate_limit INT NOT NULL);"
	...
	CREATE_OWNER_QUERY                 = "INSERT OR REPLACE INTO owner(id,email_id,rate_limit) VALUES (?,?,?);"
	GET_OWNER_DETAILS_BY_EMAILID_QUERY = "SELECT * FROM owner WHERE email_id = ?;"
	UPDATE_OWNER_RATE_LIMIT_QUERY      = "UPDATE TABLE OWNER SET rate_limit = ? WHERE email_id=?"
)
